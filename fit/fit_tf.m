function [model, h] = fit_tf(cmplx_rsp, freq, num, den, tau, data_method, fit_method, showplot)
%% fit_tf: fits tranfer function to emperical frequency response data
%
%  INPUTS:
...
switch data_method
    case 'RI' % fit to real & imaginary parts
        ...
        objfcn_str = ...
            sprintf('objfcn = @(x) [(model.R(%s model.rad) - real(Cn)).^2, (model.I(%s model.rad) - imag(Cn)).^2];', x_str, x_str);
 	case 'GP' % fit to gain & phase
        ...
        objfcn_str = ...
            sprintf('objfcn = @(x) [(model.Gain(%s model.rad) - abs(Cn)).^2, 10*(model.Phase(%s model.rad) - angle(Cn)).^2];', x_str, x_str);
    otherwise
