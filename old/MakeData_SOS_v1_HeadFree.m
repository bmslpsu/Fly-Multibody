...
%       -
%
rootdir = 'E:\EXPERIMENTS\MAGNO\Experiment_SOS_v1';
...
for kk = 1:N.file
	...
	[TRIG,PAT]  = sync_pattern_trigger(daq_time, daq_pattern, function_length, ...
...
for t = 3
    ...
    clear ax
    ax(1) = subplot(4,1,1); hold on ; ylabel('Reference(°)')
        plot(ALL{1}(trial).Time, ALL{1}(trial).refState(:,1), 'k', 'LineWidth', lw)

    ax(2) = subplot(4,1,2); hold on ; ylabel('Body(°)') ; ylim([-60 20])
        plot(ALL{1}(trial).Time, ALL{1}(trial).State(:,1), 'Color', [0.7 0 0], 'LineWidth', lw)

    ax(3) = subplot(4,1,3); hold on ; ylabel('Head(°)') ; ylim(10*[-1 1])
        plot(ALL{1}(trial).Time, ALL{1}(trial).State(:,2), 'Color', [0 0 0.7], 'LineWidth', lw)

    ax(4) = subplot(4,1,4); hold on ; ylabel('\DeltaWBA(°)') ; ylim(10*[-1 1])
        plot(ALL{1}(trial).Time, ALL{1}(trial).State(:,6), 'Color', [0.4 0 0.7], 'LineWidth', lw)
