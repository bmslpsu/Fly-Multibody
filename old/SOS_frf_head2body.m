...
%%
clc
clearvars -except HeadFree HeadFree_file
...
fig = figure (1) ; clf
set(fig, 'Color', 'w', 'Units', 'inches', 'Position', [2 2 2*n_cond 3*1.8])
movegui(fig, 'center')
clear ax h
ax = gobjects(3,n_cond);
for v = 1:n_cond
    subI = v + (0:4)*n_cond;
    ax(1,v) = subplot(3,n_cond,subI(1)); hold on ; title([num2str(HeadFree.U{1,3}{1}(v)) '°/s'], 'interpreter', 'none')
