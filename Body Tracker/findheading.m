function [heading,flip,bias,ax,fig] = findheading(frame,debug)
%% findheading: find heading of fly in grey-scale image
%
%   INPUT:
%       frame    	:   frame to extract heading
%       debug       :   show debig figure (boolean)
%   OUTPUT:
...
top4 = fix(dim(1)/3);
bot4 = 2*top4;
mask_top(1:top4,:) = true;
mask_bot(bot4:dim(1),:) = true;
...
if debug
    ...
    ax(1) = subplot(1,2,1); cla; hold on; axis image ; title(['Bias = ' num2str(bias)])
        ...
        plot([0 size(check_frame,2)], [top4 top4], 'Color', top_color, 'LineWidth', 2)
        plot([0 size(check_frame,2)], [bot4 bot4], 'Color', bot_color, 'LineWidth', 2)
        hold off
