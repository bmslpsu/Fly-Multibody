function [] = Experiment_SOS_amp(Fn)
...
amp = [1 2 3]'; % amplitude of each SOS function in order in PControl
% amp = [0.74 0.77 0.79]'; % amplitude of each SOS function in order in PControl
...
n_func = length(amp);	% # of functions
...
for ii = 1:3
    ...
    disp('Saving...')
    disp('-----------------------------------------------------------------')
    fname = ['fly_' num2str(Fn) '_trial_' num2str(ii) '_amp_' ...
        num2str(amp_all(ii)) '.mat'];
