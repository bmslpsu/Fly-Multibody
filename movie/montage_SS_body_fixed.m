function [MOV] = montage_SS_body_fixed(rootdir,rootpat,vidFs,export)
%% montage_SS_body_fixed: makes movie for fly in magnetic tether
%
...
PATH.head_track     = fullfile(PATH.raw,'tracked_head_tip');
PATH.beninfly_track	= fullfile(PATH.raw,'tracked_head_wing');
...
FLY.body    = 0*FLY.time; % body angles [°]
FLY.head    = -head_data.head_data.angle; % head angles [°]
FLY.head    = filtfilt(b,a,FLY.head); % head angles [°]
FLY.lwing   = rad2deg(hampel(FLY.time,benifly_data.RWing)); % left wing angles [°]
FLY.rwing   = rad2deg(hampel(FLY.time,benifly_data.LWing)); % right wing angles [°]
...
FLY.wing_length = 150;
FLY.lwing_hinge = [5+FLY.raw_xP , 0] + [-params.gui.right.hinge.x  , params.gui.right.hinge.y];
FLY.rwing_hinge = [5+FLY.raw_xP , 0] + [-params.gui.left.hinge.x , params.gui.left.hinge.y];
...
FLY.head_length = 38;
FLY.head_hinge = [5+FLY.raw_xP , 0] + ...
    [-head_data.head_mask.move_points.rot(1) , head_data.head_mask.move_points.rot(2)];
...
clear ax ax_pat
ax(1) = subplot(16,2,1:16) ; cla; hold on; axis square % for raw fly & pattern vid
...
head_ylim = 5*ceil(max(abs(FLY.int_head - 0*median(FLY.int_head)))./5);
set(ax(4), 'YLim', (head_ylim)*[-1 1], 'YTick', head_ylim*[-1 0 1])
set(ax(5), 'YLim', dwba_ylim*[-1 1], 'YTick', (dwba_ylim)*[-1 0 1])
set(ax(2), 'YLim', pat_ylim)
