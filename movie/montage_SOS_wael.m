function [MOV] = montage_SOS_wael(rootdir,rootpat,vidFs,export)
%% montage_SOS_wael: makes movie for fly in magnetic tether
%
...
PATH.abdomen_track     = fullfile(PATH.reg,'tracked_abdomen_tip');
...
abdomen_data  	= load(fullfile(PATH.abdomen_track,FILE.main),'abdomen_data','abdomen_mask'); % load head angles
...
FLY.body    = -180 + body_data.bAngles; % body angles [°]
FLY.head    = head_data.head_data.angle; % head angles [°]
FLY.head    = filtfilt(b,a,FLY.head); % head angles [°]
FLY.abdomen = abdomen_data.abdomen_data.angle; % abdomen angles [°]
FLY.abdomen = filtfilt(b,a,FLY.abdomen); % head angles [°]
...
FLY.int_abdomen	= FLY.abdomen(TRIG.range);
...
FLY.rwing_tip = FLY.rwing_hinge + 1.2*FLY.wing_length*[cosd(FLY.int_rwing + FLY.body_reg), ...
...
FLY.abdomen_length = 78;
FLY.abdomen_hinge = fliplr(fix(padsize)) + abdomen_data.abdomen_mask.move_points.rot;
FLY.abdomen_tip   = FLY.abdomen_hinge + FLY.abdomen_length*[sind(FLY.int_abdomen + FLY.body_glob) , ...
                    cosd(FLY.int_abdomen + FLY.body_glob)];
...
set(FIG, 'Color', 'k', 'Renderer', 'OpenGL','Position', 0.8*[100, 100, 1.75*16*40, 1.6*16*50]);
% set(FIG, 'Visible','off');
...
ax(4) = subplot(16,2,(16+4+1):(16+2*4)) ; cla; hold on
        ...
        h.body = animatedline('Color','m','LineWidth',linewidth); % for body angle
ax(5) = subplot(16,2,(16+2*4+1):(16+3*4)) ; cla; hold on
        ylabel('Abdomen (°)','Color','w','FontSize',fontsize)
        h.head = animatedline('Color','y','LineWidth',linewidth); % for head angle
        h.abdomen = animatedline('Color','c','LineWidth',linewidth); % for abdomen angle
ax(6) = subplot(16,2,(16+3*4+1):(16+4*4)) ; cla; hold on
        ...
        h.lwba = animatedline('Color','b','LineWidth',linewidth); % for left WBA angle
        h.rwba = animatedline('Color','r','LineWidth',linewidth); % for right WBA angle
...
set(ax(5), 'YLim', 10*[-1 1], 'YTick', [-5 0 5])
set(ax(6), 'YLim', [10 180])
...
for jj = 1:FLY.nframe
        ...
        set(FIG, 'CurrentAxes', ax(2)); cla; hold on; axis image
            imshow(Frame.reg)
          	...
          	%plot([FLY.head_hinge(1), mean(FLY.head_tip(win,1))], ... % update line drawn to head
                 %[FLY.head_hinge(2), mean(FLY.head_tip(win,2))], 'Color', 'y', 'LineWidth', 2)
            %plot(FLY.head_hinge(1), FLY.head_hinge(2), 'c.', 'MarkerSize', 5)
            
          	plot([FLY.abdomen_hinge(1), mean(FLY.abdomen_tip(win,1))], ... % update line drawn to abdomen
                 [FLY.abdomen_hinge(2), mean(FLY.abdomen_tip(win,2))], 'Color', 'c', 'LineWidth', 2)
            plot(FLY.abdomen_hinge(1), FLY.abdomen_hinge(2),'c.','MarkerSize',10)
                         
          	plot([FLY.lwing_hinge(1) , mean(FLY.lwing_tip(win,1))], ... % update line drawn to left wing
                [FLY.lwing_hinge(2) , mean(FLY.lwing_tip(win,2))], 'Color', 'b', 'LineWidth', 2)
            
        	plot([FLY.rwing_hinge(1) , mean(FLY.rwing_tip(win,1))], ... % update line drawn to right wing
                [FLY.rwing_hinge(2) , mean(FLY.rwing_tip(win,2))], 'Color', 'r', 'LineWidth', 2)
            
            plot(FLY.lwing_hinge(1), FLY.lwing_hinge(2),'b.','MarkerSize',10)
            plot(FLY.rwing_hinge(1), FLY.rwing_hinge(2),'r.','MarkerSize',10)
    ...
    set(FIG, 'CurrentAxes', ax(5))
        ...
        addpoints(h.abdomen, FLY.int_time(jj), FLY.int_abdomen(jj))
        
   	% WBA plot
	set(FIG, 'CurrentAxes', ax(6))
        addpoints(h.lwba, FLY.int_time(jj), 90 + FLY.int_lwing(jj))
        addpoints(h.rwba, FLY.int_time(jj), 90 + FLY.int_rwing(jj))
