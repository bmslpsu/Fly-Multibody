function [MOV] = MagnoMontage_Ramp_v1(rootdir,rootpat,vidFs,export)
...
FILE.montage    = [FILE.basename '_Montage_v3.mp4'];
...
pattern_total_time = 15;
% [TRIG,PAT] = sync_pattern_trigger(raw_data.t_p, raw_data.data(:,2), pattern_total_time, ...
%                         raw_data.data(:,1), true, false);
[TRIG,PAT] = sync_pattern_trigger(raw_data.t_p, raw_data.data(:,2), pattern_total_time, raw_data.data(:,1), ...
                true, [], false, true);

%% Get kinematics data
FLY.time    = [TRIG.time_sync;TRIG.time_sync(end) + (1/100)]; % video time
...
% FLY.head    = filtfilt(b,a,rad2deg(benifly_data.Head)); % head angles [°]
FLY.head    = FLY.head - mean(FLY.head); % head angles [°]
...
% Heading
fid = fopen(fullfile(PATH.beninfly_track, FILE.mask));
raw = fread(fid,inf); 
str = char(raw'); 
fclose(fid); 
...
set(ax(6), 'YLim', 10 *[-1 1], 'YTick', 8 *[-1 0 1])
linkaxes(ax(3:end),'x')
...
for jj = 1:FLY.nframe % for each frame
    if expframe(jj) % if we want to display this frame
        ...
        % Display raw video
        subplot(16,2,1:2:15); cla; hold on; axis square
            imshow(Frame.raw)
        ...
        subplot(16,2,2:2:16); cla; hold on; axis square
            imshow(Frame.reg)            
