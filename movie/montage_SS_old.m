function [MOV] = montage_SS(rootdir,rootpat,vidFs,export)
%% montage_SS: makes movie for fly in magnetic tether
%
...
thickness = 7; % radius display width
...
set(ax(end),'XTick', 0:1:round(FLY.int_time(end)))
set(ax(3:end-1), 'XTickLabel', [], 'XColor', 'none')
set(ax(3), 'YLim', 5*ceil(amp/5)*[-1 1], 'YTick', 5*round(amp/5)*[-1 0 1])
...
for jj = 1:FLY.nframe % for each frame
    if expframe(jj) % if we want to display this frame
        ...
        subplot(16,2,1:2:15); cla; hold on; axis image
            ...
            pat_pos = 3.75*round(mean(PAT.pos_intrp_exp(win)));
        ...
        subplot(16,2,2:2:16); cla; hold on; axis image
            imshow(Frame.reg)
