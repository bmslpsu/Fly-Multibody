...
%% Find image stats across all frames
clc
filt_vid = vid;
clear all_stats
for n = 1:n_frame
	...
	A{end+1} = uint8(imbinarize(A{end}));
    filt_vid(:,:,n) = A{end};
...
if playback
    ...
    set(fig, 'Color', [0 0 0.1], 'Units', 'inches', 'Position', [4 1 10 9])
        ...
        ax(9) = subplot(4,3,9); cla ; hold on ; axis image
        ...
        set(ax(1:10), 'XColor', 'w', 'YColor', 'w', 'Box', 'on')
        set(ax(1:9), 'Color', 'none')
        set(ax(10), 'Color', 'k', 'LineWidth', 1, 'Box', 'off')
        set(ax(1:7), 'XTick', [1 20:20:dim(2)])
        set(ax(1:7), 'YTick', [1 20:20:dim(1)])
end
...
[pks,locs,w,~] = findpeaks(-edge, 'MinPeakDistance', 10, 'MinPeakWidth', 5, ...
...
if isempty(locs)
    ...
    offset = 0.1*neck.width;

    neck.edge_range = ((neck.peak  - ceil(neck.width/2) ):floor(neck.peak - offset))';
