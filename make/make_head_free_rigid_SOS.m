function [] = make_head_free_rigid_SOS(rootdir)
%% make_head_free_rigid_SOS:
%
%   INPUTS:
...
% rootdir = 'E:\EXPERIMENTS\RIGID\Haltere_Left';

clss = 'position';
% clss = 'velocity';
filename = ['SOS_rigid_'  num2str(clss)];
...
DATA = [D(:,1:2) , splitvars(table(num2cell(zeros(N.file,7))))];
DATA.Properties.VariableNames(3:end) = {'reference','head','error',...
...
for n = 1:N.file
	...
	data.daq = load(fullfile(root.base,  [basename{n} '.mat']),'data','t_p');
	data.head = load(fullfile(root.head, [basename{n} '.mat']),'hAngles');
