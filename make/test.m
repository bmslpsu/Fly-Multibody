...
for kk = 20:N{1,end}
	...
	data.head = load(fullfile(root.head, [basename{kk} '.mat']),'yaw','roll_idx'); % load head angles
	...
	[TRIG,PAT]  = sync_pattern_trigger(daq_time, daq_pattern, func_length, trigger, true, [], true, false);
    trig_time   = TRIG.time_sync;
    
    % Get pattern, head, & body anlges
    pat = 3.75*PAT.pos;
	head.yaw = data.head.yaw;
    head.roll = 36.33*data.head.roll_idx;
    ...
    Yaw = interp1(trig_time, head.yaw,  tintrp, 'pchip');
    Yaw = Yaw - 0*mean(Yaw);
    
    Roll = interp1(trig_time, head.roll,  tintrp, 'pchip');
    Roll = Roll - 0*mean(Roll);
    
  	Yaw = hampel(tintrp,Yaw);
    Roll = hampel(tintrp, Roll);
     
    SYS_ref2head  = frf(tintrp, Reference, IOFreq, true, Yaw, Roll);
    SYS_yaw2roll  = frf(tintrp, Yaw, IOFreq, false, Roll);
    
    pause
    close all
    
    SYS_all = CatStructFields(2, SYS_ref2head, SYS_yaw2roll);
...
ax(3) = subplot(1,3,3); hold on ; title('Gaze')
    ...
    legend(hh,string(IOFreq) + " Hz",'Box','off','Location','north')
...
ax(3) = subplot(3,3,3); hold on ; title('Gaze: Body + Head')
    ...
    [h.patch(3),h.line(3)] = PlotPatch(GRAND.fly_stats.mean.IOGain.mean(:,gI),...
              ...
              1, 1, gcolor, gcolor, 0.2, 2);       
                 
ax(4) = subplot(3,3,4); hold on ; ylabel('Phase Difference (°)')
    plot([0 10], [0 0], '--k')
    plot(squeeze(GRAND.all.IOFv(:,1,:)), rad2deg(squeeze(GRAND.all.IOPhaseDiff(:,bI,:))), ...
        'Color', [0.5 0.5 0.5 0.3])
	phase = rad2deg(GRAND.fly_stats.circ_mean.IOPhaseDiff.circ_mean(:,bI));
    shiftI = (phase > 0) & ((1:5)==5)';
    phase(shiftI) = phase(shiftI) - 360;
...
%         'Color', [0.5 0.5 0.5 0.3])
%  	plot(squeeze(GRAND.all.Time(:,1,:)), mean(squeeze(GRAND.all.refState(:,rI,:)),2), ...
