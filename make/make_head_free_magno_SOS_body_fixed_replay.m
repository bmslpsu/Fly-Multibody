...
clss = 'position';
% clss = 'velocity';
filename = ['SOS_BodyFixed_Replay' exp_typ '_' exp_ver '_' num2str(clss)];
...
ax(2) = subplot(4,1,2); cla ; hold on ; ylim([-200 100])
    ...
    plot(squeeze(GRAND.all.IOFv(:,1,:)), rad2deg(squeeze(GRAND.all.IOPhaseDiff(:,pI,:))), ...
        '.-', 'MarkerSize', 10, 'Color', [0.5 0.5 0.5 0.3], 'LineWidth', 0.25)
    plot(GRAND.fly_stats(1).mean.IOFv.mean(:,1), rad2deg(GRAND.fly_stats(1).mean.IOPhaseDiff.mean(:,pI)), ...
        '.-k', 'MarkerSize', 17, 'LineWidth', 2)
