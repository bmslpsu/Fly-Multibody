function [] = make_head_free_magno_SOS_body_fixed_replay(rootdir)
%% make_head_free_magno_SOS_body_fixed_replay:
%
%   INPUTS:
...
rootdir = 'E:\EXPERIMENTS\MAGNO\Experiment_SOS_vel_v2_body_fixed_replay';
...
% clss = 'position';
clss = 'velocity';
filename = ['SOS_BodyFixed_Replay_' exp_typ '_' exp_ver '_' num2str(clss)];
...
DATA = [D , splitvars(table(num2cell(zeros(N.file,9))))];
DATA.Properties.VariableNames(4:end) = {'reference','body','head','error',...
    'dwba','lwing','rwing','body_saccade','fictive_reference'};
for n = 1:N.file
    %disp(kk)
    disp(basename{n})
    ...
    fictive_Reference = FUNC{1}.replay.pos.ref(:,vI);
    ...
    DATA.fictive_reference{n} = singal_attributes(fictive_Reference, tintrp);
    ...
    IOFreq = sort(FUNC{1}.FUNC{vI}.All.Freq, 'ascend');
    REF = DATA.reference{n}.(clss);
    FREF = DATA.fictive_reference{n}.(clss);
    ...
    SYS_fref2_head = frf(tintrp, FREF , IOFreq, false, HEAD);
    
	SYS_all = CatStructFields(2, SYS_ref2_head, SYS_err2_head, SYS_fref2_head);
...
set(ax, 'Color', 'none', 'LineWidth', 1, 'XScale', 'log', 'XLim', [0.3 12])
linkaxes(ax, 'x')
...
ax(1) = subplot(1,1,1); cla ; hold on
    ...
    cmplx = squeeze(GRAND.all.IOFRF(:,pI,:));
    R = real(cmplx);
    I = imag(cmplx);
    n_freq = size(cmplx,1);
    fI = (1:n_freq)';
    n_trial = size(cmplx,2);
    G = repmat(fI, [n_trial, 1]);
    h.trial = gscatter(R(:), I(:), G, [], '.', 8, true);
    
    cmplx = GRAND.fly_stats(1).mean.IOFRF.mean(:,pI);
    R = real(cmplx);
    I = imag(cmplx);
    %h.med = gscatter(R(:), I(:), fI, 'k', '.', 15, true);
    h.med = plot(R(:), I(:), 'k.-', 'MarkerFaceColor', 'none', ...
        'MarkerEdgeColor', 'k', 'MarkerSize', 15);
%     for f = 1:n_freq
%        h.trial(f).Color = [h.trial(f).Color 0.1]; 
%     end
    
%     plot(squeeze(GRAND.all.IOFv(:,1,:)), squeeze(GRAND.all.IOGain(:,pI,:)), ...
%         '.-', 'MarkerSize', 10, 'Color', [0.5 0.5 0.5 0.3], 'LineWidth', 0.25)
    
set(ax, 'Color', 'none', 'LineWidth', 1, 'Box', 'on', 'XLim', [-1 1], 'YLim', [-1 1])
