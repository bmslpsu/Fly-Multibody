function [sys, data, h] = switch_system_v2(P1, P2, C1, C2, delay1, delay2, colors)
...
% Compute controller that shapes P1 alone into H2
syms s H1 H2 P1 P2 C1 C2
% sys.C1_switch = subs(equations.multi.controllers.C1, [H1 H2 P1], [tf2sym(sys.H1) tf2sym(sys.H2) tf2sym(sys.P1)]);
sys.C1_switch = subs(equations.multi.controllers.C1, [H1 H2 P1], [tf2sym(sys.H2) tf2sym(sys.H1) tf2sym(sys.P1)]);
sys.C1_switch = minreal_sym(collect(simplify(expand(sys.C1_switch)),s),s);

% Recompute H1, which should now match H2
% sys.H1_switch = subs(equations.multi.closedloop.H1, [C1 C2 P1 P2], [tf2sym(sys.C1) tf2sym(sys.C2) tf2sym(sys.P1) tf2sym(sys.P2)]);
sys.H1_switch = subs(equations.multi.closedloop.H1, [C1 C2 P1 P2], [tf2sym(sys.C1) sys.C1_switch tf2sym(sys.P1) tf2sym(sys.P1)]);
...
sys.W1_switch = minreal( sys.C1_switch / (1 + sys.G1_switch + sys.G2) );

% Compute controller that shapes P2 alone into H1
sys.C2_switch = subs(equations.multi.controllers.C2, [H1 H2 P2], [tf2sym(sys.H1) tf2sym(sys.H2) tf2sym(sys.P2)]);
% sys.C2_switch = subs(equations.multi.controllers.C2, [H1 H2 P2], [tf2sym(sys.H2) tf2sym(sys.H1) tf2sym(sys.P2)]);
sys.C2_switch = minreal_sym(collect(simplify(expand(sys.C2_switch)),s),s);

% Recompute H2, which should now match H1
sys.H2_switch = subs(equations.multi.closedloop.H2, [C1 C2 P1 P2], [tf2sym(sys.C1) tf2sym(sys.C2) tf2sym(sys.P1) tf2sym(sys.P2)]);
% sys.H2_switch = subs(equations.multi.closedloop.H2, [C1 C2 P1 P2], [sys.C2_switch tf2sym(sys.C2) tf2sym(sys.P1) tf2sym(sys.P2)]);
...
sys.W2_switch = minreal( sys.C2_switch / (1 + sys.G2 + sys.G2_switch) );

%% Plant, Controller, Closed-loop, Control-input bod eplot computations   
