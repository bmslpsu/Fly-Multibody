function q = unwrap_custom(p, cutoff, dim, wrap_lim)
...
case 1
   ...
   cutoff = 90;     % Original UNWRAP used pi*170/180.
case 2
   [p,nshifts] = shiftdim(p);
otherwise    % nargin == 3
...
for j=1:size(p,2)
   ...
   q(indf,j) = LocalUnwrap(p(indf,j), cutoff, wrap_lim);
end
...
p(2:m,:) = p(2:m,:) - wrap_lim*cumsum(dp_corr,1);
