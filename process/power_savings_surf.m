...
tau_all = flipud(logspace(log10(tau_start), log10(tau_body), 40)'); % make array of time constants up to nominal value
...
animal = [];
animal.size_ratio.robot = (160e-3) / ((84 + 35 + 9)*1e-3); % Iyer
...
animal.color = distinguishable_colors(n_animal);
for n = 1:n_animal
...
h.fly.exp = plot3(fv, fly_size_ratio_grid, 100*power_ratio_fly, ...
    'k-', 'MarkerEdgeColor', 'none', 'MarkerSize', 20, 'LineWidth', 2);

leg = legend([h.animal , h.fly.exp], [fnames ; {'fly'}], 'Interpreter', 'none');
leg.Location = 'north';
