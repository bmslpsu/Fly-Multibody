...
set_names = ["HeadFree", "HeadFree", "HeadFree", "BodyFixed"];
trf_names = ["ref2body", "ref2gaze", "ref2head", "ref2head"];
...
cond = 1;
% cond = ALL.HeadFree.U{1,3};
n_cond = length(cond);
cc = [0.9 0 0; 0.5 0.3 1 ; 0 0.4 1 ; 0 0.8 0.2];
% cc = distinguishable_colors(n_set);

fig = figure (1) ; clf
set(fig, 'Color', 'w', 'Units', 'inches', 'Position', [2 2 n_cond*2.2 n_plot*1.8])
movegui(fig, 'center')
clear ax h
...
set(ax, 'Color', 'none', 'LineWidth', 0.75, 'FontSize', 10, 'XLim', [0.5 25],...
...
set(ax(1,1:end),'YLim',[0 220])
set(ax(2,1:end),'YLim',[0 1])
set(ax(3,1:end),'YLim',[-250 150])
set(ax(3,1:end),'YTick',-600:100:100)
...
set_names = ["HeadFree", "HeadFree", "BodyFixed", "HeadFree", "VirtualGaze"];
trf_names = ["ref2body", "ref2head", "ref2head", "ref2gaze", "ref2gaze"];
plot_names = ["gain", "phase", "error", "IO_coherence"];
yLines = [1 0 1 nan];
n_set = length(trf_names);
n_plot = length(plot_names);
n_cond = 1;
cc = [0.9 0 0 ; 0 0.4 1 ; 0 0.8 0.2; 0.5 0.3 1; 0.2 0.8 1];
...
for v = 1:n_cond
    ...
    for p = 1:n_plot
        ...
        for n = 1:n_set
            if ~(strcmp(set_names(n), "VirtualGaze") && strcmp(plot_names(p), "IO_coherence"))
                [h.patch(p,v,n),h.line(p,v,n)] = PlotPatch(...
                        ALL.(set_names(n)).FRF_data.(trf_names(n)).grand_mean(v).(plot_names(p)),...
                        ALL.(set_names(n)).FRF_data.(trf_names(n)).grand_std(v).(plot_names(p)), ...
                        ALL.(set_names(n)).FRF_data.IOFv, 1, 1, cc(n,:), 0.7*cc(n,:), 0.2, 1);
            else
               h.patch(p,v,n) = plot(-1, -1);
               h.line(p,v,n) = plot(-1, -1); 
            end
        end
     	uistack(squeeze(h.line(p,v,:)), 'top')
    end
...
leg = legend(squeeze(h.line(1,1,:)), leg_label, ...
    'Box', 'off', 'interpreter', 'none', 'Orientation', 'vertical');
leg.Position  = [0.27 0.11 0.7 0.12];
...
set(ax(1,1:end),'YLim',[0 1.8])
set(ax(2,1:end),'YLim',[-600 150])
set(ax(2,1:end),'YTick',-600:100:100)
