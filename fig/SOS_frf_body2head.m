...
fig = figure (1) ; clf
set(fig, 'Color', 'w', 'Units', 'inches', 'Position', [2 2 n_cond*2 n_plot*1.2])
movegui(fig, 'center')
clear ax h
...
for v = 1:n_cond
    ...
    for p = 1:n_plot
        ...
        for n = 1:n_set
            ...
            plot(ALL.(set_names(n)).FRF_data.IOFv{v}, ...
                ALL.(set_names(n)).FRF_data.(trf_names(n)).fly(v).(plot_names(p)), ...
...
set(ax(1,1:end-1),'YLim',[0 4])
set(ax(2,1:end),'YLim',[-100 200])
