...
All.dwba = splitapply(@(x) {cat(2,x{:})}, cellfun(@(x) x.mag.(clss), DATA.dwba, ...
    'UniformOutput', false), wave_fly_group);
All.body2head = splitapply(@(x) {cat(2,x{:})}, cellfun(@(x) x.coherence, DATA.body2head, ...
    'UniformOutput', false), wave_fly_group);
...
%% Plot bosy2head coherence by spatial wavelength
fig = figure (1) ; clf
set(fig, 'Color', 'w','Units', 'inches', 'Position', [2 2 7 6/3])
ax = gobjects(1,N.wave);
pp = 1;
Fv = All.Fv{1}(:,1);
for w = [2:N.wave, 1]
    subI = pp + (0:2)*N.wave;
    ax(1,pp) = subplot(1,N.wave,subI(1)); cla ; hold on ; title(num2str(U.wave{1}(w)))
        mag_mean = All.wave_stats.body2head(w).mean;        
        %plot(All.all_trial.Fv{w}, All.all_trial.body2head{w}, ...
            %'Color', [0.5 0.5 0.5 0.2], 'LineWidth', 0.25)
        plot(All.fly_mean.Fv{w}, All.fly_mean.body2head{w},...
            'Color', [0.5 0.5 0.5 0.2], 'LineWidth', 0.25)
        plot(Fv, mag_mean, 'k', 'LineWidth', 0.75)
...
YLabelHC = get(ax(1,1), 'YLabel');
set([YLabelHC], 'String', {'body-head','coherene'})
