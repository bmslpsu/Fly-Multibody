function [] = SS_time_wing2body_pos_vel_accel()
%% SS_time:
...
clc
pI = [1 5 10 11];
T = ["body position", "body velocity", "acceleration", "\DeltaWBA"];
...
for v = 1:N.freq
    ...
    %yyaxis left % body and reference
        ax(v,1) = gca; set(ax(v,1), 'YColor', 'k')
        axis tight
        %set(ax(v,1), 'YTick', FUNC{v}.All.Amp*[0 1])
        %set(ax(v,1), 'YLim', FUNC{v}.All.Amp*[-1.3 1.3])
        %set(ax(v,1), 'YLim', FUNC{v}.All.norm_vel*[-1.3 1.3])
        func = FUNC{v}.All.X(func_span);
        temp = singal_attributes(func, time, []);
        plot(func_time_span, temp.position ./ max(abs(temp.position)) , '--r', 'LineWidth', 0.5)
        plot(func_time_span, temp.velocity ./ max(abs(temp.velocity)) , '--b', 'LineWidth', 0.5)
        plot(func_time_span, temp.acceleration ./ max(abs(temp.acceleration)) , '--m', 'LineWidth', 0.5)
        
        pos_state = GRAND.fly_stats(v).mean.State.mean(data_span,pI(1));
        temp = singal_attributes(pos_state, time, 1.5*U.freq{1}(v));
        [h.patch(v,1),h.line(v,1)] = PlotPatch(temp.position ./ max(abs(temp.position)),...
                  GRAND.fly_stats(v).std.State.mean(data_span,pI(1)), time_span, ...
                  0, 1, 'r', 0.7*cc(1,:), 0.2, 0.75);
        [h.patch(v,2),h.line(v,2)] = PlotPatch(temp.velocity ./ max(abs(temp.velocity)),...
                  GRAND.fly_stats(v).std.State.mean(data_span,pI(1)), time_span, ...
                  0, 1, 'b', 0.7*cc(1,:), 0.2, 0.75);
        [h.patch(v,3),h.line(v,3)] = PlotPatch(temp.acceleration ./ max(abs(temp.acceleration)),...
                  GRAND.fly_stats(v).std.State.mean(data_span,pI(1)), time_span, ...
                  0, 1, 'm', 0.7*cc(1,:), 0.2, 0.75);
              
    %yyaxis right % wings
        %ax(v,2) = gca; set(ax(v,2), 'YColor', cc(2,:))
        plotdata = GRAND.fly_stats(v).mean.State.mean(data_span,pI(2));
        [h.patch(v,4),h.line(v,4)] = PlotPatch(plotdata ./ max(abs(plotdata)),...
                  GRAND.fly_stats(v).std.State.mean(data_span,pI(2)), time_span, ...
                  0, 1, 'k', 0.7*cc(2,:), 0.2, 0.75);
...
leg = legend(h.line(1,:), T, 'Box', 'off', 'Location', 'east', 'Orientation', 'vertical');
leg.Position = [0.0160    0.9240    0.5000    0.0666];
linkaxes(ax(:,1), 'y')
% linkaxes(ax, 'x')
set(ax(:,1), 'YLim', 1.0*[-1 1])
% set(ax(:,2), 'YLim', 1.2*[-1 1])
set(ax(:,1), 'YTick', 1*[0 1])
% set(ax(:,2), 'YTick', 1*[0 1])
set(ax, 'Color', 'none', 'LineWidth', 1, 'FontSize', 8, 'XGrid', 'off', 'YGrid', 'off', 'Box', 'off')
set(ax, 'XColor', 'none')
