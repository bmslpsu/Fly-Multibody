function [] = SOS_frf_free_fixed_replay_haltere()
%% SOS_frf_free_fixed_replay_haltere:
...
[FILE{5},PATH{5}] = uigetfile({'*.mat'}, 'Select body-free haltere cut data', root, 'MultiSelect','off');
datasets = ["HeadFree", "HeadFixed", "BodyFixed", "BodyFixed_Replay", "HaltereCut"];
for n = 1:length(FILE)
...
set_names = ["HeadFree", "HeadFixed", "HeadFree", "HeadFree", "BodyFixed", "VirtualGaze", ...
    "BodyFixed_Replay", "HaltereCut", "HaltereCut", "HaltereCut"];
trf_names = ["ref2body", "ref2body", "ref2gaze", "ref2head", "ref2head", "ref2gaze", "fref2head", "ref2body", "ref2head", "ref2gaze"];
...
cc = [0.9 0 0 ; 1 0.6 0.1 ; 0.5 0.3 1 ; 0 0.4 1 ; 0 0.8 0.2 ; 0.2 0.8 1 ; 0.1 0.5 0.7 ; 0.6 0.1 0; 0.1 0.3 0.6 ; 0.3 0.5 0.3];
% cc = distinguishable_colors(n_set);

vlist = 2;
n_cond = length(vlist);
fig = figure (1) ; clf
set(fig, 'Color', 'w', 'Units', 'inches', 'Position', 1.2*[2 2 n_cond*2.5 n_plot*1.8])
movegui(fig, 'center')
clear ax h
...
for v = 1:n_cond
    ...
    for p = 1:n_plot
        ...
        for n = 1:n_set
            if strcmp((set_names(n)), 'BodyFixed_Replay') || strcmp((set_names(n)), 'HaltereCut')
                vI = 1;
            else
                vI = vlist(v);
            end
