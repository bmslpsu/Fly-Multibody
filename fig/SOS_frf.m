...
for v = 1
    for n = 1:n_plot
        ...
        ax(3,n) = subplot(4,n_plot,subI(3)); hold on
            grand_med = 1000*GRAND.fly_stats(v).mean.IOFRF_TimeDiff.mean(fI,pI(n));
            grand_std = 1000*GRAND.fly_stats(v).std.IOFRF_TimeDiff.mean(fI,pI(n));
        ...
        ax(4,n) = subplot(4,n_plot,subI(4)); hold on
            ...
            [h.patch(4,n),h.line(4,n)] = PlotPatch(grand_med,...
