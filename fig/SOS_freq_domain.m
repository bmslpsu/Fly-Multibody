...
%         [h.patch(1,v),h.line(1,v)] = PlotPatch(GRAND.fly_stats(v).median.Mag.median(:,1),...
%                   GRAND.fly_stats(v).median.Mag.std(:,1), Fv, 0, 1, cc(1,:), 0.7*cc(1,:), 0.2, 1);
%         [h.patch(2,v),h.line(2,v)] = PlotPatch(GRAND.fly_stats(v).median.Mag.median(:,2),...
%                   GRAND.fly_stats(v).median.Mag.std(:,2), Fv, 0, 1, cc(2,:), 0.7*cc(2,:), 0.2, 1);
%         [h.patch(3,v),h.line(3,v)] = PlotPatch(GRAND.fly_stats(v).median.Mag.median(:,3),...
%                   GRAND.fly_stats(v).median.Mag.std(:,2), Fv, 0, 1, cc(3,:), 0.7*cc(3,:), 0.2, 1);
        ax(v).YLim(1) = -4;
end
set(h.line, 'Marker', '.','MarkerFaceColor', 'none', 'MarkerSize', 15, 'LineWidth', 1.5)
