function [] = SOS_frf_free_fixed_replay()
%% SOS_frf_free_fixed_replay:
...
[FILE{4},PATH{4}] = uigetfile({'*.mat'}, 'Select body-fixed replay data', root, 'MultiSelect','off');
datasets = ["HeadFree", "HeadFixed", "BodyFixed", "BodyFixed_Replay"];
for n = 1:length(FILE)
...
%% Body-fixed predicition with haltere feedback
n_cond = ALL.HeadFree.N{1,3};
for v = 1:n_cond                                             
    ALL.BodyFixed.FRF_data.ref2head_pred.grand_mean(v).complex = ALL.HeadFree.FRF_data.err2head.grand_mean(v).complex ./ ...
                                                    (1 + ALL.HeadFree.FRF_data.err2head.grand_mean(v).complex); 
                                                
	ALL.BodyFixed.FRF_data.ref2head_pred.grand_mean(v).gain = abs(ALL.BodyFixed.FRF_data.ref2head_pred.grand_mean(v).complex);
    ALL.BodyFixed.FRF_data.ref2head_pred.grand_mean(v).phase = rad2deg( angle(ALL.BodyFixed.FRF_data.ref2head_pred.grand_mean(v).complex) );
    ALL.BodyFixed.FRF_data.ref2head_pred.grand_mean(v).error = abs( (1 + 0*1i) - ALL.BodyFixed.FRF_data.ref2head_pred.grand_mean(v).complex);
    ALL.BodyFixed.FRF_data.ref2head_pred.grand_mean(v).IO_coherence = nan*ALL.BodyFixed.FRF_data.ref2head_pred.grand_mean(v).gain;
    
	ALL.BodyFixed.FRF_data.ref2head_pred.grand_std(v).gain = 0*abs(ALL.BodyFixed.FRF_data.ref2head_pred.grand_mean(v).complex);
    ALL.BodyFixed.FRF_data.ref2head_pred.grand_std(v).phase = 0*rad2deg( angle(ALL.BodyFixed.FRF_data.ref2head_pred.grand_mean(v).complex) );
    ALL.BodyFixed.FRF_data.ref2head_pred.grand_std(v).error = 0*abs( (1 + 0*1i) - ALL.BodyFixed.FRF_data.ref2head_pred.grand_mean(v).complex);
    ALL.BodyFixed.FRF_data.ref2head_pred.grand_std(v).IO_coherence = nan*ALL.BodyFixed.FRF_data.ref2head_pred.grand_mean(v).gain;
end

%% Compare head free, head-fixed, body-fixed
clc
set_names = ["HeadFree", "HeadFixed", "HeadFree", "HeadFree", "BodyFixed", "VirtualGaze", "BodyFixed_Replay", "BodyFixed"];
trf_names = ["ref2body", "ref2body", "ref2gaze", "ref2head", "ref2head", "ref2gaze", "fref2head", "ref2head_pred"];
plot_names = ["gain", "phase", "error", "IO_coherence"];
yLines = [nan 0 1 nan];
n_set = length(trf_names);
n_plot = length(plot_names);
cond = ALL.HeadFree.U{1,3}{1};
cc = [0.9 0 0 ; 1 0.6 0.1 ; 0.5 0.3 1 ; 0 0.4 1 ; 0 0.8 0.2 ; 0.2 0.8 1 ; 0.1 0.5 0.7 ; 0.1 0 0.5];
% cc = distinguishable_colors(n_set);

vlist = 2;
n_cond = length(vlist);
fig = figure (1) ; clf
set(fig, 'Color', 'w', 'Units', 'inches', 'Position', [2 2 n_cond*2.5 n_plot*1.8])
movegui(fig, 'center')
clear ax h
...
for v = 1:n_cond
    ...
    for p = 1:n_plot
        ...
        if p == 1
            title([num2str(cond(vlist(v))) '°/s'], 'interpreter', 'none')
        end
        for n = 1:n_set
            if strcmp((set_names(n)), 'BodyFixed_Replay')
                vI = 1;
            else
                vI = vlist(v);
            end
            ...
            [h.patch(p,v,n),h.line(p,v,n)] = PlotPatch(...
                    ALL.(set_names(n)).FRF_data.(trf_names(n)).grand_mean(vI).(plot_names(p)),...
                  	ALL.(set_names(n)).FRF_data.(trf_names(n)).grand_std(vI).(plot_names(p)), ...
                    ALL.(set_names(n)).FRF_data.IOFv{vI}, 1, 1, cc(n,:), 0.7*cc(n,:), 0.2, 1);
        end
    end
end
...
trf_names = ["err2body", "err2head", "err2head"];
